...
def extract_project_context(cwd):
    """Read project memory and check for permissions bloat."""
    context = {}
    if not cwd:
        return context
    ...
    if memory_file.exists():
        try:
            ...
            log.debug("Failed to read MEMORY.md: %s", e)
        ...
        try:
            ...
            if size > 20000:
                context["settings_warning"] = (
                    f"WARNING: settings.local.json is very large ({size // 1024} KB). "
                    "Possible permissions bloat."
                )
            ...
            context["settings_error"] = "ERROR: settings.local.json is corrupt (invalid JSON)."
        except OSError as e:
            log.debug("Failed to read settings.local.json: %s", e)
...
def generate_summary(metadata, messages, max_preview=500):
    ...
    if project_context:
        ...
        if "bypass_permissions" in project_context:
            lines.append(f"- **Bypass Permissions:** `{project_context['bypass_permissions']}`")
